...
class Settings(BaseSettings):
    ...
    # --- GitHub webhook (commit-hook-driven change detection) ---
    # When set, incoming /webhooks/github payloads must carry a valid
    # X-Hub-Signature-256 HMAC. Empty disables verification (local/demo).
    github_webhook_secret: str = Field(default="", alias="GITHUB_WEBHOOK_SECRET")
    # When true, a webhook first `git fetch`+resets the local clone to the pushed
    # commit before detecting. Off by default so detection never discards local
    # working-tree edits unexpectedly.
    webhook_auto_pull: bool = Field(default=False, alias="WEBHOOK_AUTO_PULL")
